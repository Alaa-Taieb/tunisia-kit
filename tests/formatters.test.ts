...
import { formatPhone, formatPhoneIntl, formatPhoneAr } from '../src/formatters/phone';
import { formatCurrency, formatCurrencyAr } from '../src/formatters/currency';
...
describe('formatPhone', () => {
  ...
  it('formats with Arabic-Indic digits', () => {
    expect(formatPhoneAr('22555111')).toBe('٢٢ ٥٥٥ ١١١');
  });

  it('returns original for invalid Arabic length', () => {
...
describe('formatPhoneIntl', () => {
  ...
  it('returns original for invalid length', () => {
    expect(formatPhoneIntl('123')).toBe('123');
  });
});
...
describe('formatCurrency', () => {
  ...
  it('handles non-finite values', () => {
    expect(formatCurrency(Infinity)).toBe('0,000 TND');
    expect(formatCurrency(-Infinity)).toBe('0,000 TND');
  });

  it('formats with Arabic-Indic digits', () => {
    expect(formatCurrencyAr(1250.5)).toBe('١ ٢٥٠،٥٠٠ د.ت');
    expect(formatCurrencyAr(0)).toBe('٠،٠٠٠ د.ت');
  });

  it('handles non-finite Arabic values', () => {
    expect(formatCurrencyAr(Infinity)).toBe('٠،٠٠٠ د.ت');
    expect(formatCurrencyAr(-Infinity)).toBe('٠،٠٠٠ د.ت');
  });

  it('handles invalid Arabic input', () => {
    expect(formatCurrencyAr(NaN)).toBe('٠،٠٠٠ د.ت');
    expect(formatCurrencyAr('invalid' as any)).toBe('٠،٠٠٠ د.ت');
  });
});
