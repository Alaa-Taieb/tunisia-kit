...
import { isValidPhone, getPhoneProvider } from '../src/validators/phone';
...
describe('getPhoneProvider', () => {
  it('returns the prefix-derived provider for valid mobiles', () => {
    expect(getPhoneProvider('22555111')).toBe('Ooredoo'); // 2x
  ...
  it('handles +216 and 00216 prefixes', () => {
    expect(getPhoneProvider('+21622555111')).toBe('Ooredoo');
    expect(getPhoneProvider('0021655123456')).toBe('Orange');
  });

  it('returns null for invalid numbers', () => {
    expect(getPhoneProvider('12345678')).toBeNull(); // invalid prefix
    expect(getPhoneProvider('2255511')).toBeNull(); // too short
    expect(getPhoneProvider('not-a-phone')).toBeNull();
    expect(getPhoneProvider(null as any)).toBeNull();
  });
});
