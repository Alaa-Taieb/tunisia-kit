...
- [x] `isValidPassport(pass)` - Validate Tunisian Passport format (1L+7D or 2L+6D)
- [x] `isValidPostalCode(code)` - Validate 4-digit Tunisian postal code (1000-9299)
- [x] `getPhoneProvider(phone)` - Prefix-derived mobile operator (MNP caveat)
...
- [x] `formatCurrencyAr(num)` - Format to `١ ٢٥٠،٥٠٠ د.ت` (Arabic-Indic digits)
- [x] `formatPhoneAr(str)` - Format to `٢٢ ٥٥٥ ١١١` (Arabic-Indic digits)
...
- [x] `POSTAL_CODE_RANGES` - Compact postal-code → governorate range table
- [x] `getBankFromRIB(rib)` - Identify bank from RIB code
- [x] `getBankFromIBAN(iban)` - Identify bank from IBAN
- [x] `getGovernorateFromPostalCode(code)` - Governorate from postal code
- [x] `getGovernorateFromDelegation(name)` - Governorate from delegation name
- [x] `getDelegationByName(name)` - Find delegation by name
...
- [ ] `getBranchFromRIB(rib)` - Get bank branch info from RIB _(Deferred)_
